...
export function PerformanceSection({ data, onChange, atencion }: Props) {
              ...
              {cols.map(c => {
                ...
                return (
                  <td key={c.key} className={`${cellClass} ${c.align === 'right' ? 'text-right' : ''}`}>
                    {isResponseMetric ? formatResponseMetric(value, secRaw, (r as any).responseTimeUnit) : (value ?? '—')}
                  </td>
  ...
  const renderStressTable = (svc: PerfServiceData) => {
    ...
    const hasSummaryData = summary && ['minutesRange', 'uvc', 'trx', 'errors', 'errorRate', 'tps', 'tProm', 'tMin', 'tMax'].some((k) => {
              ...
              <tr key={stepIdx} className="hover:bg-surface-1/50">
                {stressCols.map(c => {
                  if (c.key === '_process') {
                    if (stepIdx === 0) {
                      return (
                        <td key="_process" rowSpan={steps.length} className={`${cellClass} align-middle`}>
                          {processName}
                        </td>
                      );
                    }
