    ...
    <div className="bg-surface-1 border border-border rounded-lg p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {estadoTags.map(t => (
            <div key={t.id} className="flex items-center gap-1 bg-surface-2 rounded px-2 py-1">
              <TagBadge tag={t} />
              <button
                onClick={() => handleToggleKind(t.id)}
                className="text-[9px] text-muted-foreground hover:text-primary ml-1"
                title="Cambiar a tipo"
              >
                →tipo
              </button>
              <button
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-border">
        <input
          ...
          className="bg-surface-0 border border-border rounded px-2 py-1.5 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary flex-1"
        />
        <select
          value={newKind}
          onChange={e => setNewKind(e.target.value as 'estado' | 'tipo')}
          className="bg-surface-0 border border-border rounded px-2 py-1.5 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
        >
